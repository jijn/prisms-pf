...
private:
                ...
                [[maybe_unused]] number &vector_component_value) const override
  {
    ...
    if (index == 0 && boundary_id == RectangularMesh<2>::Boundary::Left && component == 0)
      {
              ...
              [[maybe_unused]] unsigned int solve_block_id) const override
  {
    if (solve_block_id == 1) // linear rhs
    ...
    else if (solve_block_id == 2) // post-processing
      {
        ...
        VectorGrad  stress;
        ScalarValue stress_zz(0.0);
        PlaneStrain::compute_stress(stiffness, strain, stress, stress_zz);
    ...
    if (solve_block_id == 1) // linear lhs
      {
        ...
        PlaneStrain::compute_stress<ScalarValue>(stiffness, strain, stress, stress_zz);

        variable_list.set_gradient_term(0, stress);
      }
  }

  PlaneStrain::VoigtMatrix<number> stiffness;
};
